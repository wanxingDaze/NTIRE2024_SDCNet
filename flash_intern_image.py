...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, k=None, v=None):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, -1).contiguous()
        x = self.proj(x)
        x = self.proj_drop(x)
...
class FlashInternImage(nn.Module):
    ...
    def forward_features(self, x):
        ...
        h, w = shape
        x = x.view(N, h, w, -1).contiguous()
        x = self.conv_head(x.permute(0, 3, 1, 2)).contiguous()
        x = self.avgpool(x)
        x = torch.flatten(x, 1)
        return x
    ...
    def forward_features_seq_out(self, x):
        ...
        for level_idx, level in enumerate(self.levels):
            ...
            h, w= old_shape
            seq_out.append(x_.reshape(N, h, w, -1).contiguous().permute(0, 3, 1, 2).contiguous())
        return seq_out
...
#
# model = FlashInternImage(channels=112,
#                  depths=[4, 4, 21, 4],
#                  groups=[7, 14, 28, 56]).to('cuda')
#
# # 创建一个临时的输入张量，假设输入大小为 (batch_size, channels, height, width)
# dummy_input = torch.randn(1, 3, 256, 256).to('cuda')  # 以 (224, 224) 大小的图片为例
#
# # 将输入张量传递给模型并获得输出
# output_features = model(dummy_input)
#
# # 输出特征张量的形状
# print("Output features shape:", output_features[0].shape)
# print("Output features shape:", output_features[1].shape)
# print("Output features shape:", output_features[2].shape)
# print("Output features shape:", output_features[3].shape)
